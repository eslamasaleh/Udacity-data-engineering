...
def get_cluster_details():
    """
        Get all details of cluster
    """
    config = configparser.ConfigParser()
    config.read_file(open('dwh.cfg'))
    #Key and secret token from User 
    KEY               = config.get('AWS','KEY')
    SECRET            = config.get('AWS','SECRET')
    #cluster type and number of used nodes and node type
    DWH_CLUSTER_TYPE     = config.get("DWH","DWH_CLUSTER_TYPE")
    DWH_NUM_NODES        = config.get("DWH","DWH_NUM_NODES")
    DWH_NODE_TYPE        = config.get("DWH","DWH_NODE_TYPE")
    # cluster name, pass ,port
...
def create_clients(KEY, SECRET):
    """
        Creates recources and clients need for cluster
    """
...
def create_iamrole(DWH_IAM_ROLE_NAME, iam):
    try:
        ...
        dwhRole = iam.create_role(
            ...
            Description = "Allows Redshift clusters.",
...
def create_redshift_cluster(DWH_CLUSTER_TYPE, DWH_NODE_TYPE, DWH_NUM_NODES,DWH_DB,DWH_CLUSTER_IDENTIFIER,DWH_DB_USER,DWH_DB_PASSWORD,redshift):
    """
...
if __name__=='__main__':
    ...
    DWH_ENDPOINT, DWH_ROLE_ARN, myClusterProps = create_redshift_cluster(DWH_CLUSTER_TYPE,DWH_NODE_TYPE,DWH_NUM_NODES,DWH_DB,DWH_CLUSTER_IDENTIFIER,
DWH_DB_USER,
DWH_DB_PASSWORD,redshift)
